...
class RsvpSetup {
	public static function db() {
		...
		`friendlyName` VARCHAR(100) NOT NULL 
			...
			$wpdb->insert( $table, array( "questionType" => "shortAnswer", "friendlyName" => "Short Answer" ), array(
				'%s',
				'%s'
			) );
			...
			$wpdb->insert( $table, array( "questionType" => "longAnswer", "friendlyName" => "Long Answer" ), array(
				'%s',
				'%s'
			) );
			$wpdb->insert( $table, array( "questionType" => "dropdown", "friendlyName" => "Drop Down" ), array(
				'%s',
				'%s'
			) );
		...
		if ( (int) $installed_ver < 9 ) {
			Rsvp::rsvp_install_passcode_field();
		}
