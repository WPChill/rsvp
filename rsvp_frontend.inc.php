...
class RsvpFrontEnd {
	...
	public function __construct() {
		$rsvp_form_action = htmlspecialchars(Rsvp::rsvp_getCurrentPageURL() );
	...
	static function rsvp_handle_output( $intialText, $rsvpText ) {
	...
	static function rsvp_frontend_handler( $text ) {
		global $wpdb;
		$passcodeOptionEnabled = ( Rsvp::rsvp_require_passcode() ) ? true : false;
		...
		if ( ( strtotime( $openDate ) !== false ) && ( strtotime( $openDate ) > time() ) ) {
			return static::rsvp_handle_output( $text, RSVP_START_PARA . sprintf( __( "I am sorry but the ability to RSVP for our wedding won't open till <strong>%s</strong>", 'rsvp-plugin' ), date_i18n( get_option( 'date_format' ), strtotime( $openDate ) ) ) . RSVP_END_PARA );
		}

		if ( ( strtotime( $closeDate ) !== false ) && ( strtotime( $closeDate ) < time() ) ) {
			return static::rsvp_handle_output( $text, RSVP_START_PARA . __( "The deadline to RSVP for this event has passed.", 'rsvp-plugin' ) . RSVP_END_PARA );
		}
		...
		if ( isset( $_POST['rsvpStep'] ) ) {
			...
			switch ( strtolower( $_POST['rsvpStep'] ) ) {
				case( "newattendee" ):
					return static::rsvp_handlenewattendee( $text );
					break;
				case( "addattendee" ):
					return static::rsvp_handleNewRsvp( $text );
					break;
				case( "handlersvp" ) :
					$output = static::rsvp_handlersvp( $text );
					if ( ! empty( $output ) ) {
						return $output;
					}
					break;
				case( "editattendee" ) :
					$output = static::rsvp_editAttendee( $output, $text );
					if ( ! empty( $output ) ) {
						return $output;
					}
					break;
				case( "foundattendee" ) :
					$output = static::rsvp_foundAttendee( $output, $text );
					if ( ! empty( $output ) ) {
						return $output;
					}
					break;
				case( "find" ) :
					$output = static::rsvp_find( $output, $text );
					if ( ! empty( $output ) ) {
						return $output;
					}
					break;
				case( "newsearch" ):
				default:
					return static::rsvp_handle_output( $text, static::rsvp_frontend_greeting() );
					break;
			}
		} else {
			if ( ( isset( $_REQUEST['firstName'] ) && isset( $_REQUEST['lastName'] ) ) || ( Rsvp::rsvp_require_only_passcode_to_register() && isset( $_REQUEST['passcode'] ) ) ) {
				$output = "";

				return static::rsvp_find( $output, $text );
			} else {
				return static::rsvp_handle_output( $text, static::rsvp_frontend_greeting() );
			}
		}
		throw new \Exception( "I do not know how to handle: " & $text );
	}

	static function rsvp_handlenewattendee( $text ) {
		$output = RSVP_START_CONTAINER;
		$output .= static::rsvp_frontend_main_form( 0, "addAttendee" );
		$output .= RSVP_END_CONTAINER;

		return static::rsvp_handle_output( $text, $output );
	}

	static function rsvp_handleAdditionalQuestions( $attendeeID, $formName ) {
		global $wpdb;
				...
				if ( isset( $_POST[ $formName . $q->id ] ) && ! empty( $_POST[ $formName . $q->id ] ) ) {
					if ( $q->questionType == QT_MULTI ) {
						...
						if ( ! empty( $selectedAnswers ) ) {
							$wpdb->insert( ATTENDEE_ANSWERS, array(
	...
	static function rsvp_frontend_prompt_to_edit( $attendee ) {
	...
	static function rsvp_frontend_main_form( $attendeeID, $rsvpStep = "handleRsvp" ) {
		...
		if ( ! empty( $attendee->personalGreeting ) ) {
			$form .= static::rsvp_BeginningFormField( "rsvpCustomGreeting", "" ) . nl2br( stripslashes( $attendee->personalGreeting ) ) . RSVP_END_FORM_FIELD;
		}

		// New Attendee fields when open registration is allowed
		if ( $attendeeID <= 0 ) {
			$form .= RSVP_START_PARA;
			$form .= static::rsvp_BeginningFormField( "", "" ) .
		...
		$form .= RSVP_END_PARA .
		         static::rsvp_BeginningFormField( "", "" ) .
		...
		if ( get_option( OPTION_HIDE_KIDS_MEAL ) != "Y" ) {
			$form .= static::rsvp_BeginningFormField( "", "rsvpBorderTop" ) .
			         RSVP_START_PARA . $kidsVerbiage . RSVP_END_PARA .
			         "<input type=\"radio\" name=\"mainKidsMeal\" value=\"Y\" id=\"mainKidsMealY\" " .
			         ( ( ( $attendee->kidsMeal == "Y" ) || ( isset( $rsvp_saved_form_vars['mainKidsMeal'] ) && ( $rsvp_saved_form_vars['mainKidsMeal'] == "Y" ) ) ) ? "checked=\"checked\"" : "" ) . " /> <label for=\"mainKidsMealY\">$yesText</label> " .
			         "<input type=\"radio\" name=\"mainKidsMeal\" value=\"N\" id=\"mainKidsMealN\" " . ( ( ( $attendee->kidsMeal == "Y" ) || ( isset( $rsvp_saved_form_vars['mainKidsMeal'] ) && ( $rsvp_saved_form_vars['mainKidsMeal'] == "Y" ) ) ) ? "" : "checked=\"checked\"" ) . " /> <label for=\"mainKidsMealN\">$noText</label>" .
			         RSVP_END_FORM_FIELD;
		}
		...
		if ( get_option( OPTION_HIDE_VEGGIE ) != "Y" ) {
			$form .= static::rsvp_BeginningFormField( "", "rsvpBorderTop" ) .
			         ...
			         ( ( ( $attendee->veggieMeal == "Y" ) || ( isset( $rsvp_saved_form_vars['mainVeggieMeal'] ) && ( $rsvp_saved_form_vars['mainVeggieMeal'] == "Y" ) ) ) ? "checked=\"checked\"" : "" ) . "/> <label for=\"mainVeggieMealY\">$yesText</label> " .
			         "<input type=\"radio\" name=\"mainVeggieMeal\" value=\"N\" id=\"mainVeggieMealN\" " .
			         ( ( ( $attendee->veggieMeal == "Y" ) || ( isset( $rsvp_saved_form_vars['mainVeggieMeal'] ) && ( $rsvp_saved_form_vars['mainVeggieMeal'] == "Y" ) ) ) ? "" : "checked=\"checked\"" ) . " /> <label for=\"mainVeggieMealN\">$noText</label>" .
			         RSVP_END_FORM_FIELD;
		}
		...
		$form .= static::rsvp_buildAdditionalQuestions( $attendeeID, "main" );
      ...
      id IN (SELECT waa1.attendeeID FROM " . ASSOCIATED_ATTENDEES_TABLE . " waa1
		...
		if ( count( $associations ) > 0 ) {
			...
			foreach ( $associations as $a ) {
				if ( $a->id != $attendeeID ) {
					...
					$form .= static::rsvp_BeginningFormField( "", "" ) . RSVP_START_PARA . sprintf( __( " Will %s be attending?", 'rsvp-plugin' ), esc_html( stripslashes( $a->firstName . " " . $a->lastName ) ) ) . RSVP_END_PARA .
					...
					$form .= static::rsvp_buildAdditionalQuestions( $a->id, $a->id );
		...
		static::rsvp_inject_add_guests_js( $attendeeID );
	...
	static function rsvp_revtrievePreviousAnswer( $attendeeID, $questionID ) {
		global $wpdb;
		$answers = "";
	...
	static function rsvp_buildAdditionalQuestions( $attendeeID, $prefix ) {
		...
		if ( count( $questions ) > 0 ) {
			foreach ( $questions as $q ) {
				$oldAnswer = static::rsvp_revtrievePreviousAnswer( $attendeeID, $q->id );

				$output .= static::rsvp_BeginningFormField( "", "" ) . RSVP_START_PARA . stripslashes( $q->question ) . RSVP_END_PARA;
				...
				if ( $q->questionType == QT_MULTI ) {
					...
					if ( count( $answers ) > 0 ) {
						...
						foreach ( $answers as $a ) {
							$output .= static::rsvp_BeginningFormField( "", "rsvpCheckboxCustomQ" ) . "<input type=\"checkbox\" name=\"" . $prefix . "question" . $q->id . "[]\" id=\"" . $prefix . "question" . $q->id . $a->id . "\" value=\"" . $a->id . "\" "
	...
	static function rsvp_find( &$output, &$text ) {
		global $wpdb, $rsvp_form_action;
		$passcodeOptionEnabled = ( Rsvp::rsvp_require_passcode() ) ? true : false;
		$passcodeOnlyOption    = ( Rsvp::rsvp_require_only_passcode_to_register() ) ? true : false;
		...
		if ( ! $passcodeOnlyOption && ( ( strlen( $firstName ) <= 1 ) || ( strlen( $lastName ) <= 1 ) ) ) {
			...
			$output .= static::rsvp_frontend_greeting();
		...
		if ( $attendee != null ) {
			...
			if ( strtolower( $attendee->rsvpStatus ) == "noresponse" ) {
				...
				$output .= static::rsvp_frontend_main_form( $attendee->id );
			} else {
				$output .= static::rsvp_frontend_prompt_to_edit( $attendee );
			}

			return static::rsvp_handle_output( $text, $output . "</div>\r\n" );
		}
		...
		if ( ! $passcodeOptionEnabled && ( get_option( OPTION_RSVP_DISABLE_USER_SEARCH ) != "Y" ) ) {
			for ( $i = 3; $i >= 1; $i -- ) {
				$truncFirstName = static::rsvp_chomp_name( $firstName, $i );
		...
		if ( Rsvp::rsvp_require_only_passcode_to_register() ) {
		...
		} else if ( Rsvp::rsvp_require_passcode() ) {
		...
		$notFoundText .= static::rsvp_frontend_greeting();

		return static::rsvp_handle_output( $text, $notFoundText );
	}

	static function rsvp_handleNewRsvp( &$text ) {
		global $wpdb, $rsvp_saved_form_vars;
		...
		if ( Rsvp::rsvp_require_passcode() ) {
			$rsvpPassword = trim( Rsvp::rsvp_generate_passcode() );
		...
		static::rsvp_handleAdditionalQuestions( $attendeeID, "mainquestion" );
		...
		foreach ( $associations as $a ) {
			if ( isset( $_POST[ 'attending' . $a->id ] ) && ( ( $_POST[ 'attending' . $a->id ] == "Y" ) || ( $_POST[ 'attending' . $a->id ] == "N" ) ) ) {
				...
				static::rsvp_handleAdditionalQuestions( $a->id, $a->id . "question" );
			}
		}
			...
			if ( is_numeric( $_POST['additionalRsvp'] ) && ( $_POST['additionalRsvp'] > 0 ) ) {
				for ( $i = 1; $i <= $_POST['additionalRsvp']; $i ++ ) {
					...
					if ( ( $i <= $numGuests ) &&
						...
						static::rsvp_handleAdditionalQuestions( $newAid, $i . 'question' );
		...
		return static::rsvp_handle_output( $text, static::rsvp_frontend_new_atendee_thankyou( $thankYouPrimary, $thankYouAssociated, $rsvpPassword ) );
	}

	static function rsvp_handlersvp( &$text ) {
		global $wpdb;
		$thankYouAssociated = array();
    ...
    id IN (SELECT waa1.attendeeID FROM " . ASSOCIATED_ATTENDEES_TABLE . " waa1
      ...
      WHERE waa2.associatedAttendeeID = %d AND waa1.attendeeID <> %d))";
			...
			return static::rsvp_handle_output( $text, static::frontend_rsvp_thankyou( $thankYouPrimary, $thankYouAssociated ) );
		} else {
			return static::rsvp_handle_output( $text, static::rsvp_frontend_greeting() );
		}
	}

	static function rsvp_editAttendee( &$output, &$text ) {
		global $wpdb;
		...
		if ( is_numeric( $_POST['attendeeID'] ) && ( $_POST['attendeeID'] > 0 ) ) {
			...
			if ( $attendee != null ) {
				...
				return static::rsvp_handle_output( $text, $output . RSVP_END_CONTAINER );
			}
		}
		throw new \Exception("unable to edit attendee");

	}

	static function rsvp_foundAttendee( &$output, &$text ) {
		global $wpdb;
	...
	static function frontend_rsvp_thankyou( $thankYouPrimary, $thankYouAssociated ) {
	...
	static function rsvp_frontend_new_atendee_thankyou( $thankYouPrimary, $thankYouAssociated, $password = "" ) {
	...
	static function rsvp_chomp_name( $name, $maxLength ) {
		for ( $i = $maxLength; $maxLength >= 1; $i -- ) {
			if ( strlen( $name ) >= $i ) {
				return substr( $name, 0, $i );
			}
		}
		throw new \Exception( "unable to chomp name" );
	}

	static function rsvp_BeginningFormField( $id, $additionalClasses ) {
		return "<div " . ( ! empty( $id ) ? "id=\"$id\"" : "" ) . " class=\"rsvpFormField " . ( ! empty( $additionalClasses ) ? $additionalClasses : "" ) . "\">";
	}

	static function rsvp_frontend_greeting() {
		global $rsvp_form_action;
	...
	static function rsvp_inject_add_guests_js( $attendeeID ) {
		if ( get_option( OPTION_HIDE_ADD_ADDITIONAL ) != "Y" ) {
			...
			$tmpVar = str_replace( "\r\n", "", str_replace( "||", "\"", addSlashes( static::rsvp_buildAdditionalQuestions( $attendeeID, "|| + numAdditional + ||" ) ) ) );
