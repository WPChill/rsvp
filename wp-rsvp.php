...
class Rsvp {
	...
	static function rsvp_frontend_handler($text)
    {
	    return RsvpFrontEnd::rsvp_frontend_handler( $text );
    }

	/*
	 * Description: Database setup for the rsvp plug-in.
	 */
	static function rsvp_database_setup() {
		global $wpdb;
		require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
		require_once( "rsvp_db_setup.inc.php" );
	}

	static function rsvp_install_passcode_field() {
		global $wpdb;
	...
	static function rsvp_require_passcode() {
	...
	static function rsvp_require_only_passcode_to_register() {
		return ( get_option( OPTION_RSVP_ONLY_PASSCODE ) == "Y" );
	}

	static function rsvp_require_unique_passcode() {
		return static::rsvp_require_only_passcode_to_register();
	}

	static function rsvp_is_passcode_unique( $passcode, $attendeeID ) {
		global $wpdb;
	 ...
	 */
	static function rsvp_generate_passcode() {
		$length     = 6;
	...
	function rsvp_admin_guestlist_options() {
		global $wpdb;
		if ( static::rsvp_require_unique_passcode() ) {
		...
		if ( static::rsvp_require_passcode() ) {


			static::rsvp_install_passcode_field();

			$sql       = "SELECT id, passcode FROM " . ATTENDEES_TABLE . " WHERE passcode = ''";
			$attendees = $wpdb->get_results( $sql );
			foreach ( $attendees as $a ) {
				$wpdb->update( ATTENDEES_TABLE,
					array( "passcode" => static::rsvp_generate_passcode() ),
					array( "id" => $a->id ),
					array( "%s" ),
					array( "%d" ) );
			}
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        <th scope="row"><label
                                    for="rsvp_num_additional_guests"><?php echo __( "Number of Additional Guests Allowed (default is three):", 'rsvp-plugin' ); ?></label>
                        </th>
                    ...
                    <tr valign="top">
                        ...
                        <td align="left"><?php echo __( "Default is: &quot;There are a few more questions we need to ask you if you could please fill them out below to finish up the RSVP process.&quot;", 'rsvp-plugin' ); ?>
                            <br/>
                    ...
                    <tr valign="top">
                        ...
                        <td align="left"><?php echo __( "Default is: &quot;So, how about it?&quot;", 'rsvp-plugin' ); ?>
                            <br/>
                    ...
                    <tr valign="top">
                        ...
                        <td align="left"><?php echo __( "Default is: &quot;Did we slip up and forget to invite someone? If so, please add him or her here:&quot;", 'rsvp-plugin' ); ?>
                            <br/>
                            <input type="text" name="<?php echo OPTION_RSVP_ADD_ADDITIONAL_VERBIAGE; ?>"
        ...
        <div class="wrap">
            ...
            <form method="post" id="rsvp-form" enctype="multipart/form-data">
                ...
                <input type="hidden" id="sortValue" name="sortValue"
                       value="<?php echo htmlentities( $sort, ENT_QUOTES ); ?>"/>
                <input type="hidden" name="exportSortDirection"
                       value="<?php echo htmlentities( $sortDirection, ENT_QUOTES ); ?>"/>
                <div class="tablenav">
                    <div class="alignleft actions">
                        ...
                        <input type="submit" value="<?php _e( 'Apply', 'rsvp-plugin' ); ?>" name="doaction"
                               id="doaction"
                               class="button-secondary action"
                               onclick="document.getElementById('rsvp-bulk-action').value = document.getElementById('rsvp-action-top').value;"/>
                        <input type="submit" value="<?php _e( 'Export Attendees', 'rsvp-plugin' ); ?>"
                               name="exportButton"
                    ...
                    <div class="alignright"><?php __( "RSVP Count -", 'rsvp-plugin' ); ?>
						<?php echo __( "Yes:", 'rsvp-plugin' ); ?>
                        <strong><?php echo $yesResults[0]->yesCount; ?></strong>
                        &nbsp; &nbsp; &nbsp; &nbsp;
                ...
                <table class="widefat post fixed" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <th scope="col" id="rsvpStatus" class="manage-column column-title"
                            style=""><?php echo __( "RSVP Status", 'rsvp-plugin' ); ?><br/>
                            <a href="admin.php?page=rsvp-top-level&amp;sort=rsvpStatus&amp;sortDirection=asc">
                                <img src="<?php echo plugins_url(); ?>/rsvp/uparrow<?php
								echo( ( ( $sort == "rsvpStatus" ) && ( $sortDirection == "asc" ) ) ? "_selected" : "" ); ?>.gif"
                                     width="11" height="9"
                            ...
                            <a href="admin.php?page=rsvp-top-level&amp;sort=rsvpStatus&amp;sortDirection=desc">
                                <img src="<?php echo plugins_url(); ?>/rsvp/downarrow<?php
								echo( ( ( $sort == "rsvpStatus" ) && ( $sortDirection == "desc" ) ) ? "_selected" : "" ); ?>.gif"
                                     width="11" height="9"
                                     alt="Sort Descending RSVP Status" title="Sort Descending RSVP Status"
                                     border="0"></a>
                        </th>
                        <th scope="col" id="rsvpDate"
                            class="manage-column column-title"><?php echo __( "RSVP Date", 'rsvp-pro-plugin' ); ?></th>
						<?php if ( get_option( OPTION_HIDE_KIDS_MEAL ) != "Y" ) { ?>
                            <th scope="col" id="kidsMeal" class="manage-column column-title"
                                style=""><?php echo __( "Kids Meal", 'rsvp-plugin' ); ?><br/>
                                <a href="admin.php?page=rsvp-top-level&amp;sort=kidsMeal&amp;sortDirection=asc">
                                    <img src="<?php echo plugins_url(); ?>/rsvp/uparrow<?php
									echo( ( ( $sort == "kidsMeal" ) && ( $sortDirection == "asc" ) ) ? "_selected" : "" ); ?>.gif"
                                         width="11" height="9"
                                         alt="Sort Ascending Kids Meal Status" title="Sort Ascending Kids Meal Status"
                                         border="0"></a> &nbsp;
                                <a href="admin.php?page=rsvp-top-level&amp;sort=kidsMeal&amp;sortDirection=desc">
                                    <img src="<?php echo plugins_url(); ?>/rsvp/downarrow<?php
									echo( ( ( $sort == "kidsMeal" ) && ( $sortDirection == "desc" ) ) ? "_selected" : "" ); ?>.gif"
                                         width="11" height="9"
                                         alt="Sort Descending Kids Meal Status" title="Sort Descending Kids Meal Status"
                                         border="0"></a>
                            </th>
						<?php } ?>
                        <th scope="col" id="additionalAttendee" class="manage-column column-title"
                            style=""><?php echo __( "Additional Attendee", 'rsvp-plugin' ); ?> <br/>
                            <a href="admin.php?page=rsvp-top-level&amp;sort=additional&amp;sortDirection=asc">
                                <img src="<?php echo plugins_url(); ?>/rsvp/uparrow<?php
								echo( ( ( $sort == "additional" ) && ( $sortDirection == "asc" ) ) ? "_selected" : "" ); ?>.gif"
                                     width="11" height="9"
                                     alt="Sort Ascending Additional Attendees Status"
                                     title="Sort Ascending Additional Attendees Status" border="0"></a> &nbsp;
                            <a href="admin.php?page=rsvp-top-level&amp;sort=additional&amp;sortDirection=desc">
                                <img src="<?php echo plugins_url(); ?>/rsvp/downarrow<?php
								echo( ( ( $sort == "additional" ) && ( $sortDirection == "desc" ) ) ? "_selected" : "" ); ?>.gif"
                                     width="11" height="9"
                                     alt="Sort Descending Additional Attendees Status"
                                     title="Sort Descending Additional Atttendees Status" border="0"></a>
                        </th>
						<?php if ( get_option( OPTION_HIDE_VEGGIE ) != "Y" ) { ?>
                            <th scope="col" id="veggieMeal" class="manage-column column-title"
                                style=""><?php echo __( "Vegetarian", 'rsvp-plugin' ); ?> <br/>
                                <a href="admin.php?page=rsvp-top-level&amp;sort=vegetarian&amp;sortDirection=asc">
                                    <img src="<?php echo plugins_url(); ?>/rsvp/uparrow<?php
									echo( ( ( $sort == "vegetarian" ) && ( $sortDirection == "asc" ) ) ? "_selected" : "" ); ?>.gif"
                                         width="11" height="9"
                                         alt="Sort Ascending Vegetarian Status" title="Sort Ascending Vegetarian Status"
                                         border="0"></a> &nbsp;
                                <a href="admin.php?page=rsvp-top-level&amp;sort=vegetarian&amp;sortDirection=desc">
                                    <img src="<?php echo plugins_url(); ?>/rsvp/downarrow<?php
									echo( ( ( $sort == "vegetarian" ) && ( $sortDirection == "desc" ) ) ? "_selected" : "" ); ?>.gif"
                                         width="11" height="9"
                                         alt="Sort Descending Vegetarian Status"
                                         title="Sort Descending Vegetarian Status"
                                         border="0"></a>
                            </th>
						<?php } ?>
						...
						?>
                        <th scope="col" id="associatedAttendees" class="manage-column column-title"
                            style=""><?php echo __( "Associated Attendees", 'rsvp-plugin' ); ?></th>
                    </tr>
                    </thead>
                </table>
                <div>
                    <table class="widefat post fixed" cellspacing="0">
						...
						foreach ( $attendees as $attendee ) {
                            ...
                            <tr class="<?php echo( ( $i % 2 == 0 ) ? "alternate" : "" ); ?> author-self">
                                <th scope="row" class="check-column"><input type="checkbox" name="attendee[]"
                                                                            value="<?php echo $attendee->id; ?>"/></th>
                                <td>
	...
	static function rsvp_admin_export() {
		global $wpdb;
		...
		if ( $query->have_posts() ) {
			...
			if ( static::rsvp_require_only_passcode_to_register() ) {
		...
		foreach ( $attendees as $a ) {
			...
			if ( empty( $customLinkBase ) ) {
				...
				} else if ( static::rsvp_require_passcode() ) {
	...
	function rsvp_admin_import() {
		...
		if ( count( $_FILES ) > 0 ) {
			...
			if ( $numCols >= 2 ) {
				...
				foreach ( $data as $row ) {
					if ( $i > 0 ) {
						...
						if ( static::rsvp_require_unique_passcode() && ! static::rsvp_is_passcode_unique( $passcode, 0 ) ) {
							$passcode = static::rsvp_generate_passcode();
						}
									...
									if ( is_array( $associatedUsers ) ) {
										foreach ( $associatedUsers as $au ) {
											...
											if ( is_array( $user ) && ( count( $user ) == 2 ) ) {
												...
												$wpdb->insert( ASSOCIATED_ATTENDEES_TABLE, array(
									...
									if ( count( $res ) > 0 ) {
										...
										foreach ( $private_questions as $key => $val ) {
											if ( strToUpper( $row[ $key ] ) == "Y" ) {
												$wpdb->insert( QUESTION_ATTENDEES_TABLE, array(
													"attendeeID" => $userId,
													"questionID" => $val
												),
													array( "%d", "%d" ) );
				...
				?>
                <p>
                    <strong><?php echo $count; ?></strong> <?php echo __( "total records were imported", 'rsvp-plugin' ); ?>
                    .
                </p>
                <p><?php echo __( "Continue to the RSVP", 'rsvp-plugin' ); ?> <a
                            href="admin.php?page=rsvp-top-level"><?php echo __( "list", 'rsvp-plugin' ); ?></a></p>
				<?php
			}
		} else {
			?>
            <form name="rsvp_import" method="post" enctype="multipart/form-data">
                ...
                <p><?php echo __( "Select a file in the following file format: XLS, XLSX, CSV and ODS. It has to have the following layout:", 'rsvp-plugin' ); ?>
                    <br/>
                    <strong><?php echo __( "First Name", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Last Name", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Email", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "RSVP Status", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Kids Meal", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Associated Attendees", 'rsvp-plugin' ); ?>*</strong> |
                    <strong><?php echo __( "Vegetarian", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Passcode", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Note", 'rsvp-plugin' ); ?></strong> |
                    <strong><?php echo __( "Private Question Association", 'rsvp-plugin' ); ?>**</strong>
                </p>
                <p>
          ...
          to have the guest associated with the question put a &quot;Y&quot; in the column otherwise put whatever else you want. The header name will be the &quot;private import key&quot; which is also listed below. It has the format of pq_* where * is a number.", 'rsvp-plugin' ); ?>
                <ul>
					...
					foreach ( $questions as $q ) {
						?>
                        <li><?php echo htmlspecialchars( stripslashes( $q->question ) ); ?> -
                            pq_<?php echo $q->id; ?></li>
						<?php
					}
					?>
                </ul>
	...
	function rsvp_admin_guest() {
		...
		if ( ( count( $_POST ) > 0 ) && ! empty( $_POST['firstName'] ) && ! empty( $_POST['lastName'] ) ) {
			...
			if ( static::rsvp_require_passcode() ) {
				...
				if ( static::rsvp_require_unique_passcode() && ! static::rsvp_is_passcode_unique( $passcode, $attendeeId ) ) {
					$passcode = static::rsvp_generate_passcode();
				}
				$wpdb->update( ATTENDEES_TABLE,
            ...
            <form name="contact" action="admin.php?page=rsvp-admin-guest" method="post">
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        <th scope="row"><label for="lastName"><?php echo __( "Last Name", 'rsvp-plugin' ); ?>:</label>
                        </th>
	...
	static function rsvp_get_question_with_answer_type_ids() {
		global $wpdb;
	 ...
	 * @since 2.2.8
	 */
	static function rsvp_populate_custom_question_types() {
		global $wpdb;
	...
	static function rsvp_admin_custom_question() {
		global $wpdb;

		$answerQuestionTypes = static::rsvp_get_question_with_answer_type_ids();

		static::rsvp_populate_custom_question_types();
		...
		if ( ( count( $_POST ) > 0 ) && ! empty( $_POST['question'] ) && is_numeric( $_POST['questionTypeID'] ) ) {
			...
			if ( isset( $_POST['numNewAnswers'] ) && is_numeric( $_POST['numNewAnswers'] ) &&
				...
				for ( $i = 0; $i < $_POST['numNewAnswers']; $i ++ ) {
					if ( isset( $_POST[ 'newAnswer' . $i ] ) && ! empty( $_POST[ 'newAnswer' . $i ] ) ) {
						$wpdb->insert( QUESTION_ANSWERS_TABLE, array(
            ...
            <form name="contact" action="admin.php?page=rsvp-admin-custom-question" method="post">
                ...
                <table id="customQuestions" class="form-table">
                    ...
                    <tr valign="top">
                        <th scope="row"><label for="question"><?php echo __( "Question", 'rsvp-plugin' ); ?>:</label>
                        </th>
	...
	function rsvp_modify_menu() {
		$page = add_menu_page( "RSVP Plugin",
			...
			plugins_url( "/resources/images/rsvp_lite_icon.png", RSVP_PLUGIN_FILE ) );
	...
	function rsvp_register_settings() {
		...
		wp_register_script( 'jquery_ui', static::rsvp_getHttpProtocol() . "://ajax.microsoft.com/ajax/jquery.ui/1.8.5/jquery-ui.js" );
		wp_register_style( 'jquery_ui_stylesheet', static::rsvp_getHttpProtocol() . "://ajax.microsoft.com/ajax/jquery.ui/1.8.5/themes/redmond/jquery-ui.css" );
	}
	...
	function rsvp_admin_scripts() {
		...
		wp_register_script( 'jquery_multi_select', plugins_url( '/resources/multi-select/js/jquery.multi-select.js', RSVP_PLUGIN_FILE ) );
		wp_enqueue_script( "jquery_multi_select" );
		wp_register_style( 'jquery_multi_select_css', plugins_url( "/resources/multi-select/css/multi-select.css", RSVP_PLUGIN_FILE ) );
	...
	function rsvp_init() {
		load_plugin_textdomain( 'rsvp-plugin', false, basename( dirname( __FILE__ ) ) . '/resources/languages/' );
		wp_register_script( 'jquery_validate', static::rsvp_getHttpProtocol() . "://ajax.aspnetcdn.com/ajax/jquery.validate/1.15.0/jquery.validate.min.js" );
	...
	/*
		...
		*/
	static function rsvp_getHttpProtocol() {
	...
	static function rsvp_getCurrentPageURL() {
		$pageURL = static::rsvp_getHttpProtocol();
	...
	static function rsvpshortcode_func( $atts ) {
		return RsvpFrontEnd::rsvp_frontend_handler( "rsvp-pluginhere" );
	}
}
